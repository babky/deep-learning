...
class Network:
    ...
    def construct(self):
        with self.session.graph.as_default():
            ...
            self.learning_rate = tf.train.exponential_decay(0.01, self.global_step, 10000, 0.1)

            # The real model should be here, currently only a random guess
            batch_size = tf.shape(self.observations)[0]

            hidden_layer_1 = tf_layers.fully_connected(self.observations, num_outputs=8, activation_fn=tf.nn.relu)
            dropout_layer_1 = tf.nn.dropout(hidden_layer_1, self.dropout_keep_probability)
            hidden_layer_2 = tf_layers.fully_connected(dropout_layer_1, num_outputs=8, activation_fn=tf.nn.relu)
    ...
    def train(self, observations, actions, summaries=False, run_metadata=False):
        ...
        args = {"feed_dict": {self.observations: observations, self.actions: actions, self.training_flag: True, self.dropout_keep_probability: 0.5}}
...
if __name__ == "__main__":
    ...
    for epoch in range(10):
        ...
        # Train the network
        train_network(network, observations, actions, 15000 if epoch == 0 else 3, epoch == 0)
        ...
        if best_score >= 2000:
            ...
            observations += best_observations[0:(len(best_observations)//20)*20]
            actions += best_actions[0:(len(best_observations)//20)*20]
